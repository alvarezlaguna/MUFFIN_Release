...
REGISTER_BOUNDARY_CONDITION("Periodic", PeriodicBC);

CellDataRef PeriodicBC::setBoundary(){
    vector<Cell1D>& cells      = MeshData::getInstance().getData<Cell1D>("Cells");
    for (int iEq = 0; iEq < NBEQS; iEq++){
        if(m_side == "Right"){
            m_value[iEq]   = cells[0].uCC[iEq];   // Outlet
        }
        else if(m_side == "Left"){
            m_value[iEq]   = cells[NBCELLS - 1].uCC[iEq];             // Inlet
        }
        // DEBUGGING COMMENTS
