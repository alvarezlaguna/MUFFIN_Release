...
#include "APWBTwoFluidIsothermal.hpp"

// Self-register with the factory
#include "TimeMethodRegistrar.hpp"
REGISTER_TIMEMETHOD("APWBTwoFluidIsothermal", APWBTwoFluidIsothermal);
...
void APWBTwoFluidIsothermal::setup(){
...
double APWBTwoFluidIsothermal::velocityFlux(const CellDataRef uL, const CellDataRef uR) {
    ...
    return 0.5*(v_eL + v_eR) - 0.5*sqrt(MASSRATIO)*fa*(n_eR - n_eL)*(n_eR + n_eL)/2.;
    
    //return 0.5*(v_eL + v_eR) - 0.5*sqrt(MASSRATIO)*fa*(1./n_eR - 1./n_eL)*n_average;
...
double APWBTwoFluidIsothermal::velocityFlux_Euler(const CellDataRef uL, const CellDataRef uR) {
    const double n_eL = uL[0];
    const double n_eR = uR[0];
    const double v_eL = uL[1]/n_eL;
    const double v_eR = uR[1]/n_eR;
    ...
    //Balanced
    //double Mach12 = abs(u_12/sqrt(MASSRATIO));
    //double M_co   = (1./sqrt(MASSRATIO));
    //double fa     = sqrt((1 - M_co)*(1 - M_co)*Mach12*Mach12 + 4*M_co*M_co)/(1 + M_co*M_co);
...
double APWBTwoFluidIsothermal::pressureFlux(const CellDataRef uL, const CellDataRef uR) {
...
double APWBTwoFluidIsothermal::densityNumericalViscosity(const double niP1,const double ni, const double niM1){
...
void APWBTwoFluidIsothermal::computeElectronDensity()
{
    setBoundaries();
...
void APWBTwoFluidIsothermal::computeElectricPotential(){
...
void APWBTwoFluidIsothermal::computeElectronVelocity(){
...
//    // Test change the boundaries here
//    const double pi   = atan(1)*4;
//    const double soundSpeed_e  = SOUNDSPEED[0];
//    double randomFlux = -m_density_nP1Minus[0]*soundSpeed_e/sqrt(2*pi);
//    m_uInlet[0]  = m_density_nP1Minus[0];
//    m_uInlet[1] = 2*randomFlux - cells[0].uCC[1];
//    // Test
//    randomFlux = m_density_nP1Minus[n1]*soundSpeed_e/sqrt(2*pi);
//    m_uOutlet[0]  = m_density_nP1Minus[n1];
//    m_uOutlet[1] = 2*randomFlux - cells[n1].uCC[1];
//    cout<<"m_uInlet[0] = "<<m_uInlet[0]<<"\n";
//    cout<<"m_uInlet[1] = "<<m_uInlet[1]<<"\n";
//    cout<<"m_uOutlet[0] = "<<m_uOutlet[0]<<"\n";
//    cout<<"m_uOutlet[1] = "<<m_uOutlet[1]<<"\n";
...
void APWBTwoFluidIsothermal::computeEulerianStep(){
...
double APWBTwoFluidIsothermal::computeIonizationConstant(){
...
double APWBTwoFluidIsothermal::WBmassSourceL(const double u_12, const double c_12){
...
double APWBTwoFluidIsothermal::WBmassSourceR(const double u_12, const double c_12){
...
double APWBTwoFluidIsothermal::WBmomSourceL(const double u_12, const double c_12){
...
double APWBTwoFluidIsothermal::WBmomSourceR(const double u_12, const double c_12){
...
void APWBTwoFluidIsothermal::computeSources(){
    vector<Cell1D>& cells = MeshData::getInstance().getData<Cell1D>("Cells");
    ...
    double gradPhi_im12  = (phi[0] - (2*PHIIN - phi[0]))/Dx;
...
//    // DEBUGGING COMMENTS
//    cout<<"iCell = "<<0<<"\t\tgradPhi_im12 = "<<gradPhi_im12<<"\t\tgradPhi_ip12 = "<<gradPhi_im12<<"\n";
//    cout<<"iCell = "<<0<<"\t\tMassLorentzL = "<<WBmassSourceL(u_i_im12, sound_i)<<"\t\tMomLorentzL = "<<WBmomSourceL(u_i_im12, sound_i)<<"\n";
//    cout<<"iCell = "<<0<<"\t\tMassLorentzR = "<<WBmassSourceR(u_i_im12, sound_i)<<"\t\tMomLorentzR = "<<WBmomSourceR(u_i_im12, sound_i)<<"\n";
//    cout<<"iCell = "<<0<<"\t\tMassLorentz = "<<MassLorentz<<"\t\tMomLorentz = "<<MomLorentz<<"\n";
    
    // Compute Ionization Constant with an integration
    double ionization = nu_iz*rho_e;
    
    source[0*NBCELLS + 0] = ionization;
    source[1*NBCELLS + 0] = (- collConstElec*rho_e*u_e);
    source[2*NBCELLS + 0] = ionization - MassLorentz;
...
//        // DEBUGGING COMMENTS
//        cout<<"iCell = "<<iCell<<"\t\tgradPhi_im12 = "<<gradPhi_im12<<"\t\tgradPhi_ip12 = "<<gradPhi_im12<<"\n";
//        cout<<"iCell = "<<iCell<<"\t\tMassLorentzL = "<<WBmassSourceL(u_i_im12, sound_i)<<"\t\tMomLorentzL = "<<WBmomSourceL(u_i_im12, sound_i)<<"\n";
//        cout<<"iCell = "<<iCell<<"\t\tMassLorentzR = "<<WBmassSourceR(u_i_im12, sound_i)<<"\t\tMomLorentzR = "<<WBmomSourceR(u_i_im12, sound_i)<<"\n";
//        cout<<"iCell = "<<iCell<<"\tMassLorentz = "<<MassLorentz<<"\tMomLorentz = "<<MomLorentz<<"\n";
        // Compute Ionization Constant with an integration
        ionization = nu_iz*rho_e;
        
        source[0*NBCELLS + iCell] = ionization;
        source[1*NBCELLS + iCell] = (- collConstElec*rho_e*u_e);
        source[2*NBCELLS + iCell] = ionization - MassLorentz;
    ...
    gradPhi_ip12  = ((2*PHIOUT - phi[n1]) - phi[n1])/Dx;
    sound_i = SOUNDSPEED[1];
    // WB Lorentz contribution
    MassLorentz = gradPhi_im12*rho_i_im12*WBmassSourceL(u_i_im12, sound_i) + gradPhi_ip12*rho_i_ip12*WBmassSourceR(u_i_ip12, sound_i);
    MomLorentz  = gradPhi_im12*rho_i_im12*WBmomSourceL(u_i_im12, sound_i) + gradPhi_ip12*rho_i_ip12*WBmomSourceR(u_i_ip12, sound_i);
//    // DEBUGGING COMMENTS
//    cout<<"iCell = "<<n1<<"\t\tgradPhi_im12 = "<<gradPhi_im12<<"\t\tgradPhi_ip12 = "<<gradPhi_im12<<"\n";
//    cout<<"iCell = "<<n1<<"\t\tMassLorentzL = "<<WBmassSourceL(u_i_im12, sound_i)<<"\t\tMomLorentzL = "<<WBmomSourceL(u_i_im12, sound_i)<<"\n";
//    cout<<"iCell = "<<n1<<"\t\tMassLorentzR = "<<WBmassSourceR(u_i_im12, sound_i)<<"\t\tMomLorentzR = "<<WBmomSourceR(u_i_im12, sound_i)<<"\n";
//    cout<<"iCell = "<<n1<<"\tMassLorentz = "<<MassLorentz<<"\tMomLorentz = "<<MomLorentz<<"\n";
    // Compute Ionization Constant with an integration
    ionization = nu_iz*rho_e;
    ...
    source[0*NBCELLS + n1] = ionization;
    source[1*NBCELLS + n1] = (- collConstElec*rho_e*u_e);
    source[2*NBCELLS + n1] = ionization - MassLorentz;
    source[3*NBCELLS + n1] = (-MomLorentz - collConstIons*rho_i*u_i);
    
}


void APWBTwoFluidIsothermal::setBoundaries(){
...
void APWBTwoFluidIsothermal::computeIonFlux(){
...
void APWBTwoFluidIsothermal::takeStep(double dt){
    APWBTwoFluidIsothermal::takeStep();
}

void APWBTwoFluidIsothermal::takeStep()
{
