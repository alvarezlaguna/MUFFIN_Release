//
//  APWBTestTwoFluidIsothermal.cpp
//  Muffin
//
...
#include "APWBTestTwoFluidIsothermal.hpp"

// Self-register with the factory
#include "TimeMethodRegistrar.hpp"
REGISTER_TIMEMETHOD("APWBTestTwoFluidIsothermal", APWBTestTwoFluidIsothermal);
...
void APWBTestTwoFluidIsothermal::setup(){
...
double APWBTestTwoFluidIsothermal::velocityFlux_pred(const CellDataRef uL, const CellDataRef uR) {
...
double APWBTestTwoFluidIsothermal::velocityFlux(const CellDataRef uL, const CellDataRef uR) {
    const double n_eL = uL[0];
    const double n_eR = uR[0];
    const double v_eL = uL[1]/n_eL;
    const double v_eR = uR[1]/n_eR;
    ...
    return 0.5*(v_eL + v_eR) - 0.5*sqrt(MASSRATIO)/nMax*fa*(n_eR - n_eL);
    //return 0.5*(v_eL + v_eR) - 0.5*sqrt(MASSRATIO)*fa*(n_eR - n_eL);
    
}

double APWBTestTwoFluidIsothermal::pressureFlux(const CellDataRef uL, const CellDataRef uR) {
    ...
    return 0.5*MASSRATIO*(n_eR + n_eL) - 0.5*sqrt(MASSRATIO)*nMax*fa*(v_eR - v_eL);
    
}
double APWBTestTwoFluidIsothermal::densityNumericalViscosity(const double niP1,const double ni, const double niM1){
...
void APWBTestTwoFluidIsothermal::computeElectronDensity()
{
    setBoundaries();
...
void APWBTestTwoFluidIsothermal::computeElectricPotential(){
...
void APWBTestTwoFluidIsothermal::computeElectronVelocity(){
    ...
    m_velocity[0] = u_ei - DtOvDx*(p_ip12 - p_im12)/n_ei + Dt*MASSRATIO*m_gradPhi[0];
    ...
    for (unsigned int iCell = 1; iCell < n1; ++iCell)
    {
        ...
        m_velocity[iCell] = u_ei - DtOvDx*(p_ip12 - p_im12)/n_ei + Dt*MASSRATIO*m_gradPhi[iCell];
    ...
    m_velocity[n1] = u_ei - DtOvDx*(p_ip12 - p_im12)/n_ei + Dt*MASSRATIO*m_gradPhi[n1];
    m_momentum_nP1Minus[n1] = m_density_nP1Minus[n1]*m_velocity[n1];
    
    // Correction step to ensure mass Conservation.
    // First cell
    n_ei = cells[0].uCC[0];
    double u_ip12   = velocityFlux_pred(cells[0].uR, cells[1].uL);
    double u_im12   = velocityFlux_pred(m_uInlet, cells[0].uL);
    m_density_nP1Minus[0] = m_density_nP1Minus[0]/(1 + DtOvDx*(u_ip12 - u_im12));
    
    for (unsigned int iCell = 1; iCell < n1; ++iCell)
    {
        n_ei = cells[iCell].uCC[0];
        u_ip12   = velocityFlux_pred(cells[iCell].uR, cells[iCell + 1].uL);
        u_im12   = velocityFlux_pred(cells[iCell - 1].uR, cells[iCell].uL);
        m_density_nP1Minus[iCell] = m_density_nP1Minus[iCell]/(1 + DtOvDx*(u_ip12 - u_im12));
    }
    // Last cell
    n_ei     = cells[n1].uCC[0];
    u_ip12   = velocityFlux_pred(cells[n1].uR, m_uOutlet);
    u_im12   = velocityFlux_pred(cells[n1 - 1].uR, cells[n1].uL);
...
void APWBTestTwoFluidIsothermal::computeEulerianStep(){
...
double APWBTestTwoFluidIsothermal::computeIonizationConstant(){
...
void APWBTestTwoFluidIsothermal::computeSources(){
    vector<Cell1D>& cells = MeshData::getInstance().getData<Cell1D>("Cells");
    double* source = MeshData::getInstance().get2DData<double>("source").mutable_data(0,0);
...
void APWBTestTwoFluidIsothermal::setBoundaries(){
...
void APWBTestTwoFluidIsothermal::computeIonFlux(){
...
void APWBTestTwoFluidIsothermal::takeStep(double dt){
    APWBTestTwoFluidIsothermal::takeStep();
}

void APWBTestTwoFluidIsothermal::takeStep()
{
