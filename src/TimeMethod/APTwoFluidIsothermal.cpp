//
//  APTwoFluidIsothermal.cpp
//  Muffin
//
//  Created by Alejandro Alvarez Laguna on 31/10/18.
//  Copyright © 2018 Alejandro Alvarez Laguna. All rights reserved.
//

#include "APTwoFluidIsothermal.hpp"

// Self-register with the factory
#include "TimeMethodRegistrar.hpp"
REGISTER_TIMEMETHOD("APTwoFluidIsothermal", APTwoFluidIsothermal);
...
void APTwoFluidIsothermal::setup(){
...
double APTwoFluidIsothermal::velocityFlux(const CellDataRef uL, const CellDataRef uR) {
    ...
    //Balanced
    const double Mach12 = abs(u_12/sqrt(MASSRATIO));
...
double APTwoFluidIsothermal::pressureFlux(const CellDataRef uL, const CellDataRef uR) {
...
double APTwoFluidIsothermal::densityNumericalViscosity(const double niP1,const double ni, const double niM1){
...
void APTwoFluidIsothermal::computeElectronDensity()
{
    setBoundaries();
...
//    }
    ...
    factor      = (1 + Dt*Dt*n_ii*MASSRATIO*DEBYELENGTH*DEBYELENGTH);
    
    m_inverseDensity[0] = (1/n_ei)*(1 + DtOvDx*(u_ip12 - u_im12) - numVisc)/factor;
    ...
    for (unsigned int iCell = 1; iCell < n1; ++iCell)
    {
        ...
        m_inverseDensity[iCell] = (1/n_ei)*(1 + DtOvDx*(u_ip12 - u_im12) - numVisc)/factor;
    ...
    m_inverseDensity[n1] = (1/n_ei)*(1 + DtOvDx*(u_ip12 - u_im12) - numVisc)/factor;
...
void APTwoFluidIsothermal::computeElectricPotential(){
...
void APTwoFluidIsothermal::computeElectronVelocity(){
...
void APTwoFluidIsothermal::computeEulerianStep(){
...
double APTwoFluidIsothermal::computeIonizationConstant(){
...
void APTwoFluidIsothermal::computeSources(){
    vector<Cell1D>& cells = MeshData::getInstance().getData<Cell1D>("Cells");
    double* source = MeshData::getInstance().get2DData<double>("source").mutable_data(0,0);
...
void APTwoFluidIsothermal::setBoundaries(){
...
void APTwoFluidIsothermal::computeIonFlux(){
...
void APTwoFluidIsothermal::takeStep(double dt){
    APTwoFluidIsothermal::takeStep();
}

void APTwoFluidIsothermal::takeStep()
{
