...
void DataWriter1DExtendedMPI::writeData(const int iter)
{
    int world_size;
    ...
    sprintf(fname + strlen(fname), "/file_iter_%06i_time_%0.8e.txt", m_iter, physTime); //TODO writing the name in a general way
    ...
    for (int iCell = 0; iCell < NBCELLS; ++iCell){
        ...
        int sizeOfLine = count*(NBEQS + 2) + 1; // The full line is the x plus the variables plus the newline character
        // Write the x
        ...
        // Write the variables
        count = sprintf(line, "%*.8e\t", 18, phi[iCell]); // Line printing the result
        displace += count*sizeof(char); // start of the view for each processor
        err = MPI_File_write_at(fh,displace, line, strlen(line), MPI_CHAR, MPI_STATUS_IGNORE);
