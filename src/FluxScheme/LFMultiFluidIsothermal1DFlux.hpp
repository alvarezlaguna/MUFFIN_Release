//
//  LFMultiFluidIsothermal1DFlux.hpp
//  Muffin
//
...
#ifndef LFMultiFluidIsothermal1DFlux_hpp
#define LFMultiFluidIsothermal1DFlux_hpp
...
class LFMultiFluidIsothermal1DFlux : public FluxScheme
{
public:
    LFMultiFluidIsothermal1DFlux(string name) : FluxScheme(name), m_flux(NBEQS),
    ...
    m_fluxModels(NBFLUIDS, vector<double>(2, 0.))
    {
        for(unsigned int iFluid = 0; iFluid < NBFLUIDS; iFluid++) {
            m_fluxSchemes[iFluid].reset(new LaxFriedrichFlux("LaxFriedrichFlux"));
             // We create a pointer to the fluxes of each
        }
    }
    ~LFMultiFluidIsothermal1DFlux(){}
...
#endif /* LFMultiFluidIsothermal1DFlux_hpp */
