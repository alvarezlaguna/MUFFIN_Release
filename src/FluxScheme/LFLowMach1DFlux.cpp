...
#include <cmath>
using namespace std;
REGISTER_FLUXSCHEME("LFLowMach1D", LFLowMach1DFlux);


vector<double>& LFLowMach1DFlux::operator()(const CellDataRef uL, const CellDataRef uR)
{
    ...
    double Mach12 = abs(vBar/cBar);
    double M_co   = 0.001;//(1./cBar);
    double M_o   = sqrt(min(1., max(Mach12*Mach12, M_co*M_co)));
    double fa = (M_o*M_o + M_o)/(M_o*M_o + 1);
    
    const double lambda1        = vBar - fa*cBar;
    const double lambda2        = vBar;
    const double lambda3        = vBar + fa*cBar;
    ...
    double MaxEigenVal = max(abs(lambda1), abs(lambda3));
    
    // Lax Friedrich Flux
    m_flux[0] = 0.5*(MassFlux_R + MassFlux_L) - 0.5*MaxEigenVal*deltaMass;
    m_flux[1] = 0.5*(MomFlux_R + MomFlux_L) - 0.5*MaxEigenVal*deltaMom;
    m_flux[2] = 0.5*(EnergyFlux_R + EnergyFlux_L) - 0.5*MaxEigenVal*deltaEnergy;
